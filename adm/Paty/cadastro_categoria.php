	...
	<form method="post">
		<table border="1">
			...
			<?php
				...
				else if(isset($_POST["acao"])&&( $_POST['acao']=='inserir')){
					$sql = "insert into categoria (nome) values ('".$_POST["nome"]."');";
					//echo $sql;
					mysql_query($sql, $link);
				}
				
				if(isset($_POST["buscar"])&&( $_POST['buscar']=='buscar por codigo')){
					$sql    = "select codigo,nome FROM categoria where codigo=".$_POST["codigo"]."  order by codigo asc";
					//echo $sql;
					mysql_query($sql, $link);
				}
				else if(isset($_POST["buscar"])&&( $_POST['buscar']=='buscar por nome')){
