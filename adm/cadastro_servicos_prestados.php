...
if ($_POST) {
	...
	if ($acao=='alterar'){
		if ($imagem==null){
			$sql = "update servicos_prestados set  titulo='".$titulo."',descricao='".$descricao."' where (codigo=0".$codigo.");";
			mysql_query($sql, $link);
		}
		else{
			$sql = "update servicos_prestados set imagem='".$imagem."', titulo='".$titulo."',descricao='".$descricao."' where (codigo=0".$codigo.");";
			mysql_query($sql, $link);
			
			
			
			if ($imagem_antiga==null){
		...
		$sql = "insert into servicos_prestados (imagem, titulo, descricao) values ('".$imagem."','".$titulo."','".$descricao."');";
		//echo $sql;
		mysql_query($sql, $link);
	}
	else if ($acao=='excluir'){
		//delete_file($folder . $imagem_antiga);
		$sql = 'delete FROM servicos_prestados where codigo=0'.$codigo;
		//echo $sql;
		mysql_query($sql, $link);
	}
...
?>
	...
	<form  method="post" enctype="multipart/form-data" name="form1" id="form1">
		...
		<?php
			...
			$sql    = "SELECT codigo,titulo,descricao,imagem FROM servicos_prestados s order by titulo asc;	";
			$result = mysql_query($sql, $link);
			$i=0;
			if ($result!=null) 
		...
		?>
				...
				<tr>
					...
					<td>
						<?php if($row['imagem']!=""){?>
						<?php if ($row['titulo']!=null) echo $row['titulo'];?><br>
						<a href="../upload/servicos_prestados/h_<?php echo $row['imagem'];?>" target="_blank">ampliar<a/><br>
